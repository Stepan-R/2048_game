...
function setGame() {
    ...
    for(let i = 0; i < ROWS_NUMBER; i++)for(let j = 0; j < COLUMNS_NUMBER; j++){
...
function setNewElement() {
    ...
    while(!found){
        ...
        if (board[i][j] === 0) {
            ...
            if (randomNum === 0) board[i][j] = 4;
            else board[i][j] = 2;
            const fieldCell = document.getElementById(i.toString() + "-" + j.toString());
            fieldCell.innerText = board[i][j].toString();
            fieldCell.classList.add("field-cell--" + board[i][j]);
            found = true;
        }
    }
    if (gameOver()) messageLose.classList.remove("hidden");
}
...
function slide(originalRow) {
    ...
    newRow = deleteZero(newRow);
    while(newRow.length < COLUMNS_NUMBER)newRow.push(0);
    return newRow;
}
function slideLeft() {
    let boardChanged = false;
    for(let i = 0; i < ROWS_NUMBER; i++){
        ...
        row = slide(row);
        board[i] = row;
        if (!boardChanged && !originalRow.every((val, index)=>val === row[index])) boardChanged = true;
...
function gameOver() {
    ...
    for(let i = 0; i < ROWS_NUMBER; i++)for(let j = 0; j < COLUMNS_NUMBER - 1; j++){
...
function gameWin() {
    for(let i = 0; i < ROWS_NUMBER; i++){
        for(let j = 0; j < COLUMNS_NUMBER; j++)if (board[i][j] === 2048) {
